describe("Basic user flow for SPA ", () => {
  ...
  it("Test3: Clicking first <journal-entry>, new URL should contain /#entry1", async () => {
    await page.click("journal-entry:first-of-type");
    expect(page.url()).toBe(`http://127.0.0.1:5500/#entry1`);
  });

  it("Test4: On first Entry page - checking page header title", async () => {
    // implement test4: Clicking on the first journal entry should update the header text to “Entry 1”
    const header = await page.$eval("header h1", (h) => h.textContent);
  ...
  it("Test6: On first Entry page - checking <body> element classes", async () => {
    ...
    const classname = await page.$eval("body", (b) => b.getAttribute("class"));
  ...
  it("Test13: On home page - checking <body> element class", async () => {
    const classname = await page.$eval("body", (b) => b.getAttribute("class"));
  ...
  it("Test17: Clicking the heading title, new URL should be the home page", async () => {
    await page.click("header h1");
    expect(page.url()).toBe(`http://127.0.0.1:5500/`);
  });

  // create your own test 18
  it("Test18: Clicking the fifth entry - checking URL", async () => {
    await page.click("journal-entry:nth-child(5)");
    expect(page.url()).toBe(`http://127.0.0.1:5500/#entry5`);
  });

  // create your own test 19
  it("Test19: On fifth entry, check if content is correct", async () => {
    const entry = await page.$eval("entry-page", (e) => e.entry);
    expect(entry).toEqual({
      title: "We know each other! He’s a friend from work!",
      date: "4/29/2021",
      content:
      ...
      image: {
        src: "https://imagesvc.meredithcorp.io/v3/mm/image?url=https%3A%2F%2Fstatic.onecms.io%2Fwp-content%2Fuploads%2Fsites%2F6%2F2019%2F07%2Fthorragnarok5974d7a5729ab-2000.jpg",
        alt: "four friends looking at front camera",
      },
    });
  }, 10000);

  // create your own test 20
  it("Test20: Clicking the back button - heading should be 'Journal Entries'", async () => {
    await page.goBack();
    const header = await page.$eval("header h1", (h) => h.textContent);
    expect(header).toBe("Journal Entries");
  });
